...
var bridge;
ed.Accessorylist = [];
ed.removeDevice = (serialnumber) => {
    bridge.removeBridgedAccessory(ed.Accessorylist.filter(x => x.serialNumber == serialnumber)[0].accessory,false);
    ed.Accessorylist = ed.Accessorylist.filter(x => x.serialNumber != serialnumber);
    ed.log("Removed " + serialnumber + " from HomeKit");
}
